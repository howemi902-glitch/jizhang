...
## 13. 浏览器端 Demo 实现概览
- **目标**：提供一个可离线运行的浏览器版记账 Demo，用于演示核心交互并支持数据导入导出。
- **技术栈**：纯 HTML/CSS/JavaScript，使用 `localStorage` 持久化数据，便于直接双击打开，无需构建步骤。
- **核心能力**：
  1. 账目录入：支持收入/支出类型、金额、分类、日期与备注；使用 `crypto.randomUUID()` 生成唯一 ID。
  2. 列表与筛选：按日期倒序展示记录，可按分类、起止日期筛选；空列表时提供提示。
  3. 数据统计：实时汇总总收入、总支出与结余，格式化为人民币金额展示。
  4. 导入导出：一键导出 JSON 备份，支持从本地文件导入并校验格式；导入失败会提示错误原因。
  5. 数据安全：所有数据保存在浏览器本地，可通过导出备份迁移至其他设备。
- **发布方式**：
  - 将 `web/` 目录上传至任意静态托管平台（如 GitHub Pages、Vercel、阿里云 OSS），即可通过浏览器访问。
  - 或直接将 `web/` 打包为压缩文件分发，用户解压后双击 `index.html` 即可离线运行。
- **后续扩展**：可引入 PWA 能力（Service Worker + Web App Manifest）以支持“添加到主屏幕”、离线缓存与版本更新提示。
